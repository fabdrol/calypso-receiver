...
const server = {
  ...
  async putDelta (correlationId, path, value) {
    ...
    endpoint += '/signalk/v1/api/vessels/self/'
    endpoint += path.replace(/\./g, '/')

    debug(`[sending] ${endpoint} => ${value}`)
    
    const response = await axios.put(endpoint, { source, value }, {
      auth: {
    ...
    debug(`[sent] ${response.status} ${response.statusText}`)
  }
}
...
module.exports = function discover () {
  ...
  const browser = bonjour.findOne(query, (host) => {
    if (host && typeof host === 'object') {
      debug(`Found host: ${JSON.stringify(host.addresses || {}, null, 2)}`)
      server.server = host
      server.found = true
    }
  })
